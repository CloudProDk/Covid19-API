...
exports.get_cart_with_items_by_uuid = function (req, res) {
  ...
  //sql query
  const query =
    "SELECT cart_item.product, product.name, product.price, product.quantity, product.quantity * product.price AS total_price FROM cart INNER JOIN cart_item ON cart.id = cart_item.cart INNER JOIN product ON cart_item.product = product.id WHERE cart.uuid = ? AND cart_item.product = product.id";
